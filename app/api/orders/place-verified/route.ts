...
export async function POST(request: Request) {
    try {
        ...
        try {
            ...
            // 4. Send Email (Only if we just promoted it, or just blindly send?)
            // Better to only send if we promoted it to avoid spam on re-clicks
            if (!isAlreadyPromoted && customerDetails?.email) {
                try {
                    // Retrieve details for email if needed, or pass from frontend. 
                    // Ideally we construct email from DB data to be safe.
                    // For now, simple success notification.
