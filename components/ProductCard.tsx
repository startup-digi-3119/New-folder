...
const ProductCard = memo(function ProductCard({ product, onSelect }: ProductCardProps) {
    ...
    return (
        <Link href={`/product/${product.id}`} className="block group">
            <div
                onClick={(e) => {
                    // If onSelect is provided (modal mode), we can stop propagation to prevent navigation
                    // BUT user asked for "link redirects to product". 
                    // So maybe we just allow navigation on click? 
                    // OR: middle click / ctrl+click works via Link, left click opens modal?
                    // Let's allow default Link behavior for now, but if onSelect exists, we use it?
                    // Actually, if we wrap in Link, left click will navigate.
                    // To keep Modal: e.preventDefault() if onSelect exists.
                    if (onSelect) {
                        e.preventDefault();
                        onSelect(product);
                    }
                }}
                className="bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-slate-100 cursor-pointer active:scale-[0.99] relative h-full flex flex-col"
            >
